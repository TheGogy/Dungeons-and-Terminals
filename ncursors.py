...
class DungeonsAndTerminals():
    ...
    def init_variables(self):
        self.player = Player("Player", ["Sword", "Shield", "Library Card"])
        self.game = Game("You are an adventurer. You are on the road to the Tall Castle", "Road")
        self.is_stats = True
        self.prompt_text = ""
        self.situation_text = "You are an adventurer. You are on the road to the Tall Castle"
    ...
    def update_statistics(self):
        ...
        health_bar_top = round((1 - self.player.health / 100) * bar_height)
        ...
        self.info_win.addstr(info_y - 2,bar_width + round((bar_width - len(str(self.player.health)))/2), str(self.player.health))

        #STAMINA
        self.info_win.attron(curses.color_pair(2))
        stamina_bar_top = round((1 - self.player.stamina / 100) * bar_height)
        ...
        self.info_win.addstr(info_y - 2, 3 * bar_width + round((bar_width - len(str(self.player.stamina)))/2), str(self.player.stamina))
    ...
    def update_inventory(self):
        ...
        for x in range(len(self.player.inventory)):
            self.info_win.addstr((x * 2) + 2,2, f"{self.get_icon(self.player.inventory[x])} {self.player.inventory[x]}")
    ...
    def update_output_text(self):
        self.situation_text = self.game.history
        ...
        while True:
            ...
            if key != -1:
                if key == 127:
                    ...
                    if not self.game.run(self.player, self.prompt_text): break
                    self.update_output_text()
                    self.prompt_text = ""
                    self.update_info()
                elif key == 2:
